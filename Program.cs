...
namespace Assignment
{
    class Program
    {
        ...
        public static void deleteCustomer()
        {
            int id;
            Console.Clear();
            //check empty customer List
            string checkFirstCust = eCoord.getCustomerInfoById(200);
            if (checkFirstCust != "There is no customer with id 200.")
            {
                ...
                if (eCoord.deleteCustomer(id))
                {
                    ...
                    Console.WriteLine("Events were booking by customer with id {0} deleted too..", id);
                }
                else
                {
        ...
        {
            int id;
            string checkFirstCust, cust;
            Console.Clear();
            checkFirstCust = eCoord.getCustomerInfoById(200);
            // check if the first customer ID unavailable --> empty list
            if (checkFirstCust != "There is no customer with id 200.")
            {
                Console.WriteLine(eCoord.customerList());
            ...
            {
                Console.Write("Customer List is empty! ");
            }
        ...
        {
            int id;
            string checkFirstEvent, ev;
            Console.Clear();
            // check if the first event ID unavailable --> empty list
            checkFirstEvent = eCoord.getEventInfoById(101);
            if (checkFirstEvent != "There is no event with id 101.")
            {
                Console.WriteLine(eCoord.eventList());
            ...
            {
                Console.Write("Event list is empty!");
            }
            Console.WriteLine("\nPress any key to continue return to the previous menu.");
            Console.ReadKey();
        }
        ...
        public static void addRegister()
        {
            ...
            string checkFirstCust, checkFirstEvent;
            Console.Clear();

            //check empty Customer List and Event List
            checkFirstCust = eCoord.getCustomerInfoById(200);
            checkFirstEvent = eCoord.getEventInfoById(101);
            if ((checkFirstEvent != "There is no event with id 101.") && (checkFirstCust != "There is no customer with id 200."))
            {
                Console.WriteLine(eCoord.customerList());
                Console.WriteLine(eCoord.eventList());
            ...
            {
                Console.WriteLine("Please add customer and event before RSVP !");
            }
            Console.WriteLine("\nPress any key to continue return to the main menu.");
            Console.ReadKey();
        }
        ...
        {
            int id;
            string checkFirstRSVP, re;
            Console.Clear();
            // check if the first RSVP ID unavailable --> empty list
            checkFirstRSVP = eCoord.getRegisterInfoById(100);
            if (checkFirstRSVP != "There is no RSVP with id 100.")
            {
                ...
                id = getIntChoice();
                Console.Clear();
                re = eCoord.getRegisterInfoById(id);
                Console.WriteLine(re);
            }
            else
            {
                Console.Write("RSVP List is empty!");
            }
            Console.WriteLine("\nPress any key to continue return to the previous menu.");
            Console.ReadKey();
        }
        ...
        {
            Console.Clear();
            eCoord = new EventCoordinator(200, 1000, 101, 5000, 100, 10000); // add data
            runProgram();
